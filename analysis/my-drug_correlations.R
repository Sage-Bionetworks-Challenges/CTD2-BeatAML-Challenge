...
if(use.reticulate) {
    ...
    auc_file <- synGet("syn21212720")$path
}
aucs <- suppressMessages(read_csv(auc_file))

# Create pseudo-drug (mean response), GRD
grd <- ddply(aucs, .variables = c("lab_id"),
             .fun = function(df) { data.frame(inhibitor = "GRD", auc = mean(df$auc), stringsAsFactors = FALSE) })

grd <- grd[, colnames(aucs)]
...
if(use.reticulate) {
    ...
    rnaseq_file <- synGet("syn21212718")$path
}
rnaseq <- suppressMessages(read_csv(rnaseq_file)) %>%
  select(-Symbol) %>%
  column_to_rownames("Gene")

rnaseq_symbol <- suppressMessages(read_csv(rnaseq_file)) %>%
    select(-Gene)
...
drug.long <- melt(as.matrix(drug.mat))
colnames(drug.long) <- c("sample", "drug", "auc")
gene.long <- melt(as.matrix(rnaseq))
colnames(gene.long) <- c("gene", "sample", "expr")

drugs <- unique(drug.long$drug)
names(drugs) <- drugs

genes <- unique(gene.long$gene)
names(genes) <- genes
...
make.plot.dendro <- function(mat, sig.vec, perf.vec, cor.mat, drug.gene.cor.mat, column_dend, ...) {
    breaks = c(1, 10, 100, 1000)
    col_ha = HeatmapAnnotation(sig = anno_barplot(log10(as.numeric(sig.vec)),
                                                  axis_param = list(at = log10(breaks), labels =breaks)),
                               perf = anno_barplot(perf.vec))
...
##                 column_names_gp = gpar(fontsize = 8),
    ...
    h2 <- Heatmap(cor.mat, cluster_rows = FALSE, cluster_columns = column_dend, ...)
    h3 <- Heatmap(drug.gene.cor.mat, cluster_rows = FALSE, cluster_columns = column_dend, ...)
    h %v% h2 %v% h3
}
...
kegg.gene.sets <- dlply(kegg, .variables = c("gs_name"), .fun = function(df) as.character(df$gene_symbol))

suppressPackageStartupMessages(library("GSVA"))

es <- gsva(as.matrix(rnaseq_symbol), kegg.gene.sets, parallel.sz=num.processes, verbose=TRUE)
...
es.cors <-
    ddply(drug.long, .variables = c("drug"),
          .parallel = TRUE,
          .fun = function(drug.df) {
              ddply(es.long, .variables = c("pathway"),
                    .fun = function(gene.df) {
                        df <- merge(drug.df, gene.df)
                        ct <- cor.test(df$auc, df$es, method="pearson", use="pairwise.complete.obs")
                        ret <- data.frame(pval = as.numeric(ct$p.value), cor = as.numeric(ct$estimate))
                    })
              })
...
column_dend = as.dendrogram(hclust(dist(drug.cor.mat[drugs,drugs], method="euclidean"), method="complete"))
...
make.plot.dendro(drug.family.mat[, drugs], num.non.na.cols[drugs], drug.means[drugs], drug.cor.mat[drugs, drugs], sig.cor.pathway.mat.all[, drugs], column_dend, column_split = 6,
                 column_title_gp = gpar(fill = c("red", "blue", "green", "orange", "purple", "yellow"), font = 1:6))

## column_names_gp = gpar(col = c("red", "blue", "green", "orange", "purple"), fontsize = c(10, 10, 10, 10, 10)))
