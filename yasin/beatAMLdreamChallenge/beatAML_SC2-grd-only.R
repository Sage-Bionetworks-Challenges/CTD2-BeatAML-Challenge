...
multiv_formula <- paste('Surv(overallSurvival, vitalStatus) ~', 
                                   paste(rownames(res[which(as.numeric(res[,4])<0.05),]),collapse=" + "))
multiv_formula <- paste0(multiv_formula, " + ", "mean_auc")

# mean_auc only
multiv_formula <- paste('Surv(overallSurvival, vitalStatus) ~ mean_auc') 
...
saveRDS(coxph.fit, file="coxph-fit-mean-auc-only.rds")

ggforest(coxph.fit, data=response_data, fontsize = 1.05)
ggsave("sc2-forest-mean-auc-only.png", width = 14)